...
- **Response Export** - Converts responses to FHIR QuestionnaireResponse format
...
<div className="p-8 bg-slate-50 rounded-lg text-center">
  <h3 className="mt-0">Interactive Renderer Demo</h3>
  <p>Experience the full-featured renderer with a sample patient health survey.</p>
  <a href="/renderer-demo" className="button button--primary button--lg">Launch Renderer Demo →</a>
</div>
...
Convert responses to standard FHIR QuestionnaireResponse:

```jsx
import { buildQuestionnaireResponse } from '@mieweb/forms-renderer';

const fhirResponse = buildQuestionnaireResponse(fields, questionnaireId, subjectId);
```
...
- [FHIR Export](/docs/renderer/fhir-export) - Export QuestionnaireResponse
