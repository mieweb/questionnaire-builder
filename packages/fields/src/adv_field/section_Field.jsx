...
import { PLUSSQUARE_ICON } from "@questionnaire-builder/icons";

const SectionField = React.memo(function SectionField({ field, hooks }) {
  const ctrl = useFieldController(field, undefined, hooks);
  
  // Use hooks from props
  const ui = hooks?.useUIApi?.() || { selectedChildId: { ParentId: null, ChildId: null }, state: { isPreview: false } };
  const useFormStore = hooks?.useFormStore || (() => ({ byId: {} }));
  ...
  const renderChildPreview = (child, sectionId) => {
    ...
    return (
      <div key={child.id} ref={(el) => el && (childRefs.current[child.id] = el)}>
        <Comp field={child} sectionId={sectionId} hooks={hooks} />
      </div>
    );
  };
  ...
  const renderChildEdit = (child, sectionId) => {
    ...
    return (
      ...
      >
        <ChildField field={child} sectionId={sectionId} hooks={hooks} />
      </div>
    );
  };
