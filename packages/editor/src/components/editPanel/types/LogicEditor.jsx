...
export default function LogicEditor({ hooks }) {
  const ui = hooks.useUIApi();

  // stable store slices (hooks always called)
  const byId = hooks.useFormStore((s) => s.byId);
  const setEnableWhen = hooks.useFormStore((s) => s.setEnableWhen);
  const updateField = hooks.useFormStore((s) => s.updateField);
