...
import OptionListEditor from "./OptionListEditor.jsx";
import CommonEditor from "./CommonEditor.jsx";
import DraftIdEditor from "./DraftIdEditor.jsx";
import { fieldTypes } from "@questionnaire-builder/fields";

function SectionEditor({ section, onActiveChildChange, hooks }) {
  const sectionApi = hooks.useFormApi(section.id);
  ...
  const onUpdateChild = useCallback(
    (key, value) => {
      ...
      if (key === "id") {
        ...
        hooks.useFormStore.getState().updateField(
          activeChild.id,
          { id: next },
          {
  ...
  const onDeleteChild = useCallback(() => {
    if (!activeChild) return;
    hooks.useFormStore.getState().deleteField(activeChild.id, { sectionId: section.id });
  }, [activeChild, section.id]);
