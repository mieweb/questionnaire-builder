...
export default function Layout({ hooks, selectedField }) {
      ...
      <div className={`grid grid-cols-1 ${cols} gap-3 h-full items-start`}>
        {editMode && (
          <div className="hidden lg:block">
            <ToolPanel hooks={hooks} isPreview={isPreview} />
          </div>
        )}

        <div>
          <FormBuilderEditor hooks={hooks} />
        </div>

        {editMode && (
          <div className="hidden lg:block">
            <EditPanel key={panelResetKey} hooks={hooks} />
          </div>
        )}
