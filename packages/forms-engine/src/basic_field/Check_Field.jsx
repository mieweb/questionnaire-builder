  ...
  return (
    <FieldWrapper ctrl={ctrl}>
      {({ api, isPreview, insideSection, field: f, placeholder }) => {
        if (isPreview) {
          return (
            <div className={`check-field-preview ${insideSection ? "mie:border-b mie:border-gray-200" : "mie:border-0"}`}>
                ...
                <div className="mie:space-y-2">
                  {(f.options || []).map((option) => (
                    <label key={option.id} className="mie:flex mie:items-center mie:px-3 mie:py-2 mie:cursor-pointer mie:rounded-lg mie:hover:bg-blue-50 mie:transition-colors mie:gap-2">
                      ...
                      />
                      <span className="mie:wrap-break-word mie:overflow-hidden">{option.value}</span>
                    </label>
                  ))}
                </div>
              </div>
